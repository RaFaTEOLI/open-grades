                ...
                <div class="form-group">
                    ...
                    @foreach ($roles as $role)
                        <form class="btn" action="{{ route('users.role', ["userId" => $user->id, "roleId" => $role->id]) }}" method="post">
                            @csrf
                            @method('PATCH')
                            <button class="btn btn-success" type="submit">{{ $role->display_name }} <i class="fa fa-plus"></i></button>
                        </form>
                    @endforeach
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    @foreach ($user->roles as $role)
                        <form class="btn" action="{{ route('users.role.remove', ["userId" => $user->id, "roleId" => $role->id]) }}" method="post">
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-primary" type="submit">{{ $role->display_name }} <i class="fa fa-times"></i></button>
                        </form>
                    @endforeach
                    </div>
                </div>
